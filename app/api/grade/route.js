...
export async function POST(req) {
  ...
  if (!prompt) return Response.json({ status: 'error', feedback: 'חסר prompt' }, { status: 400 })
  ...
  if (result.error) return Response.json({ status: 'error', feedback: 'שגיאה: ' + result.error.message }, { status: 500 })

  return Response.json({ status: 'success', feedback: result.candidates[0].content.parts[0].text })
}
